...
class CEUCMEPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color categoryColor =
        const Color(0xFF789D1C); // Change to the desired color
    String categoryImagePath =
        "assets/images/icons/6.png"; // Change to the desired image path

    return FutureBuilder(
      future: fetchCEUCMEData(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          // If there's no data, show a message indicating no CEU/CME data
          return const Text('No CEU/CME data available');
        } else {
  ...
  Future<List<Map<String, dynamic>>> fetchCEUCMEData() async {
    try {
      // Get the current user's ID
      String? userId = AuthenticationService().getCurrentUserId();

      if (userId != null) {
        // Fetch CEU/CME data for the current user from Firestore
